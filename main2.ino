...
// *** USER PARAMETERS ***
#define MY_CALLSIGN "SP7WKY"      // callsign here will display on line 2 when otherwise blank (tks Richard, VE3YSH)
...
struct userparameters {
  ...
  bool vfo_high = true;                            // whether the VFO is on the high side (true) or on the low side (false) of the IF
};
...
#define RUN_TUNERANGE (2)   // set the range of the tuning pot
#define RUN_CWOFFSET (3)    // set the CW offset (=sidetone pitch)
#define RUN_SCAN (4)        // frequency scanning mode
#define RUN_SCAN_PARAMS (5) // set scan parameters
#define RUN_MONITOR (6)     // frequency scanning mode
#define RUN_FINETUNING (7)  // fine tuning mode
...
// this routine allows the user to set the six CW related parameters
...
void scan() {
  ...
  if (abs(knob - old_knob) > 8 || digitalRead(PTT_SENSE) || !digitalRead(FBUTTON) || (u.cap_sens == 0 && !digitalRead(KEY)) || (u.cap_sens != 0 && capaKEY) || !digitalRead(SPOT)) {
    //stop scanning
...
void setup() {
  ...
  if (!digitalRead(FBUTTON) || (old_version != u.raduino_version)) {
    factory_settings();
  }

  //configure the PTT SENSE input
...
void loop() {
  switch (RUNmode) {
    ...
    case 1: //calibration
      calibrate();
      break;
    case 2: // set tuning range
      set_tune_range();
      break;
    case 3: // set CW parameters
      set_CWparams();
      if (u.cap_sens != 0)
        touch_key();
      checkCW();
    ...
    case 4: // scan mode
      scan();
      break;
    case 5: // set scan paramaters
      scan_params();
      break;
    case 6: // A/B monitor mode
      scan();
      break;
    case 7: // Fine tuning mode
      finetune();
      break;
  }
  delay(100);
